    ...
    <template v-slot:form>
      <div>
        ...
        <div class="radio-group">
          <input id="preOpenKitchenYes" v-model="pre_open_kitchen" type="radio" :value="true" />
          <label for="preOpenKitchenYes">使用する</label>
        </div>
        <div class="radio-group">
          <input id="preOpenKitchenNo" v-model="pre_open_kitchen" type="radio" :value="false" />
      ...
      <div>
        <h3>調理場：営業中</h3>
        <div class="radio-group">
          <input id="duringOpenKitchenYes" v-model="during_open_kitchen" type="radio" :value="true" />
          <label for="duringOpenKitchenYes">使用する</label>
        </div>
        <div class="radio-group">
          <input id="duringOpenKitchenNo" v-model="during_open_kitchen" type="radio" :value="false" />
...
export default {
  ...
  watch: {
    cookingProcessOrder: {
      ...
      handler() {
        ...
        this.food_product_id = cookingProcessOrder.food_product_id || this.cookingProcessOrder?.food_product?.id || null;
  ...
  methods: {
    getCookingProcessOrder() {
      return this.cookingProcessOrder?.cooking_process_order || this.cookingProcessOrder || {};
    },
    async edit() {
      if (this.pre_open_kitchen === null || this.during_open_kitchen === null || this.tent === "") {

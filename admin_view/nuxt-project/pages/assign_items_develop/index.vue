      ...
      <aside class="order-group">
        ...
        <div class="order-group-content">
          <div 
           ...
           <div class="group-fulfilled-summary" v-else>
             <div class="assign-result">搬入元内訳</div>
             <div v-for="info in getGroupAssignmentDetails(group.id)" :key="info.roomName" class="assigned">
               <div class="inventory-room-name">{{ info.roomName }}より</div>
               <div class="inventory-lavel">{{ info.details }}</div>
             </div>
           </div>
          </div>
        </div>
  ...
  methods: {
    ...
    getGroupAssignmentDetails(groupId) {
          ...
          return groupAllocs.map((alloc) => {
            ...
            // 「机: 2, 椅子: 4」のような文字列を作る
            const itemDetails = [];
            for (const itemId in alloc.assigned) {
              if (activeIds.includes(Number(itemId))) {
                const count = alloc.assigned[itemId];
                if (count > 0) {
                  const item = this.items.find((i) => Number(i.id) === Number(itemId));
                  itemDetails.push(`${item ? item.name : '不明'}: ${count}`);
                }
              }
            }
          ...
          }).filter(a => a.details !== ""); // 対象物品の割り当てが0の場所は除外
        },
  },
};
</script>

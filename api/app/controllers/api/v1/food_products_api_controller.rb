class Api::V1::FoodProductsApiController < ApplicationController
  ...
  def get_refinement_food_products
    ...
    # fes_year_idだけ指定
      ...
      FoodProduct.preload(:group).filter_map { |food_product| food_product if food_product.group.fes_year_id == fes_year_id }
    # is_cookingだけ指定
    elsif fes_year_id == 0 && is_cooking != 0 && category_id == 0
      FoodProduct.where(is_cooking: is_cooking_list[is_cooking])
    # category_idだけ指定
    elsif fes_year_id == 0 && is_cooking == 0 && category_id != 0
      FoodProduct.preload(:group).filter_map { |food_product| food_product if food_product.group.group_category_id == category_id }
    # fes_year_idとis_cookingの指定
    elsif fes_year_id != 0 && is_cooking != 0 && category_id == 0
      FoodProduct.where(is_cooking: is_cooking_list[is_cooking]).filter_map { |food_product| food_product if food_product.group.fes_year_id == fes_year_id }
    # fes_year_idとcategory_idの指定
    elsif fes_year_id != 0 && is_cooking == 0 && category_id != 0
      FoodProduct.preload(:group).filter_map { |food_product| food_product if food_product.group.fes_year_id == fes_year_id && food_product.group.group_category_id == category_id }
    # is_cookingとcategory_idの指定
    elsif fes_year_id == 0 && is_cooking != 0 && category_id != 0
      FoodProduct.where(is_cooking: is_cooking_list[is_cooking]).filter_map { |food_product| food_product if food_product.group.group_category_id == category_id }
    # 全部指定
    else
      FoodProduct.where(is_cooking: is_cooking_list[is_cooking]).filter_map { |food_product| food_product if food_product.group.fes_year_id == fes_year_id && food_product.group.group_category_id == category_id }
                     end

    if search_word.present?
      # 文字列検索
      @food_products = @food_products.filter_map { |food_product| food_product if food_product.group.name.include?(search_word) || food_product.name.include?(search_word) }
    end
  ...
  def get_search_food_products
    word = params[:word]
    @food_products = FoodProduct.all.filter_map { |food_product| food_product if food_product.group.name.include?(word) || food_product.name.include?(word) }
    if @food_products.count == 0
      render json: fmt(not_found, [], 'Not found food_products')
    else
      render json: fmt(ok, fit_food_product_index_for_admin_view(@food_products))

class Api::V1::OutputCsvController < ApplicationController
  ...
  def output_rental_orders_csv
    if params[:fes_year_id].to_i == 0
      ...
      @rental_orders = Group.where(fes_year_id: params[:fes_year_id]).preload(:rental_orders).map do |group|
        group.rental_orders
      end
      filename_year = FesYear.find(params[:fes_year_id]).year_num
    end
  ...
  def  output_power_orders_csv
    if params[:fes_year_id].to_i == 0
      ...
      @power_orders = Group.where(fes_year_id: params[:fes_year_id]).preload(:power_orders).map do |group|
        group.power_orders
      end
      filename_year = FesYear.find(params[:fes_year_id]).year_num
    end
  ...
  def output_place_orders_csv
    if params[:fes_year_id].to_i == 0
      ...
      @place_orders = Group.where(fes_year_id: params[:fes_year_id]).preload(:place_order).map do |group|
        group.place_order
      end
      filename_year = FesYear.find(params[:fes_year_id]).year_num
    end
  ...
  def output_stage_orders_csv
    if params[:fes_year_id].to_i == 0
      ...
      @stage_orders = Group.where(fes_year_id: params[:fes_year_id]).preload(:stage_orders).map do |group|
        group.stage_orders
      end
      filename_year = FesYear.find(params[:fes_year_id]).year_num
    end
  ...
  def output_stage_common_options_csv
    if params[:fes_year_id].to_i == 0
      ...
      @stage_common_options = Group.where(fes_year_id: params[:fes_year_id]).preload(:stage_common_option).map do |group|
        group.stage_common_option
      end
      filename_year = FesYear.find(params[:fes_year_id]).year_num
    end
    bom = "\uFEFF"
  ...
  def output_food_products_csv
    if params[:fes_year_id].to_i == 0
      ...
      @food_products = Group.where(fes_year_id: params[:fes_year_id]).preload(:food_products).map do |group|
        group.food_products
      end
      filename_year = FesYear.find(params[:fes_year_id]).year_num
    end
  ...
  def output_purchase_lists_csv
    if params[:fes_year_id].to_i == 0
      ...
      @purchase_lists = FoodProduct.preload(:purchase_lists).map do |food_product|
        food_product.purchase_lists if food_product.group.fes_year_id == params[:fes_year_id]
      end
      filename_year = FesYear.find(params[:fes_year_id]).year_num
    end
    bom = "\uFEFF"
  ...
  def output_public_relations_csv
    if params[:fes_year_id].to_i == 0
      ...
      @public_relations = Group.where(fes_year_id: params[:fes_year_id]).preload(:public_relation).map do |group|
        group.public_relation
      end
      filename_year = FesYear.find(params[:fes_year_id]).year_num
    end
    bom = "\uFEFF"

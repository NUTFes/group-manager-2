...
module Api
  module V1
    class OutputCsvController < ApplicationController
      require 'csv'
      ...
      def output_employees_csv
        if params[:fes_year_id].to_i.zero?
          @employees = Group.preload(:employees).map(&:employees)
          filename_year = '全'
        else
          @employees = Group.where(fes_year_id: params[:fes_year_id]).preload(:employees).map(&:employees)
          filename_year = FesYear.find(params[:fes_year_id]).year_num
        end
        bom = "\uFEFF"
        csv_data = CSV.generate(bom) do |csv|

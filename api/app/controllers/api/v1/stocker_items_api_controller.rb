...
module Api
  module V1
    class StockerItemsApiController < ApplicationController
      def get_stocker_items_with_remaining_num
        ...
        # 両方ともALL
        if rental_item_id.zero? && stocker_place_id.zero?
          @stocker_items = StockerItem.all

        # rental_item_idだけ指定
        elsif rental_item_id != 0 && stocker_place_id.zero?
          @stocker_items = StockerItem.where(rental_item_id: rental_item_id)

        # stocker_place_idだけ指定
        elsif rental_item_id.zero? && stocker_place_id != 0

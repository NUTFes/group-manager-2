...
class CookingProcessOrdersController < ApplicationController
  ...
  def index
    @cooking_process_orders = current_user_group_scope(CookingProcessOrder)
  ...
  def upsert
    ...
    upserts = raw_orders.map do |order|
      ...
      food_product = current_user_group_scope(FoodProduct).find_by(id: attrs[:food_product_id])
      return render_cooking_process_order_not_found unless food_product
      return render_cooking_process_order_not_found if attrs[:id].present? && !current_user_group_scope(CookingProcessOrder).exists?(id: attrs[:id])
    ...
    scopes = upserts.map do |attrs|
      if attrs[:id].present?
        current_user_group_scope(CookingProcessOrder).where(id: attrs[:id])
      else
        current_user_group_scope(CookingProcessOrder).where(
  ...
  def set_cooking_process_order
    @cooking_process_order = current_user_group_record!(CookingProcessOrder, params[:id])
  end
  ...
  def existing_cooking_process_orders(raw_orders)
    ...
    records_by_id =
      current_user_group_scope(CookingProcessOrder)
      .where(id: ids)

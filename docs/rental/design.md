...
| 項目 | 内容 |
| --- | --- |
| 版 | v0.3（v0.2 からの変更: absolute_adjustment の集計挙動を明文化） |
| 日付 | 2026-09-09 |
| 作成 | haruto-kamijo |
| 状態 | レビュー中 |
...
| absolute_adjustment の運用 | 誰がいつ使うか。 | v1 の UI からは出さない。訂正は API 直叩き、または管理画面の将来機能とする。累計の上書き挙動は 5 章で確定（Slack で確認済み）。 | **要合意** |
| 返却済の訂正手段 | `absolute_adjustment` は貸出済の訂正としてのみ扱うため、返却数を訂正する手段が無い。カテゴリに区分の情報が無いのが原因。 | v1 は返却の訂正を持たない。必要なら `absolute_adjustment` に対象区分を持たせるか、`return_adjustment` を足す。 | **要合意** |

...
| 項目 | 内容 |
| --- | --- |
| 版 | v0.5（v0.4 からの変更: カテゴリを4値に、ステッパーとメモの仕様を明文化、緊急時の割当変更をスコープ外に） |
| 日付 | 2026-09-09 |
| 作成 | haruto-kamijo |
| 状態 | レビュー中 |
...
| F11 | 記録の訂正 UI（デザイン待ち） | 現在の累計を正しい値に直し、モードに応じて `rental_adjustment` / `return_adjustment` で記録する。誤入力を当日その場で直せるようにする。 | F6, F7, A5、デザイン追加 |

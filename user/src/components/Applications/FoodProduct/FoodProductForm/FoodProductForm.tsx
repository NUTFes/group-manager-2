...
const FoodProductForm: FC<FoodProductFormProps> = ({
    ...
    const {
        ...
        removeProduct,
        products,
    } = useFoodProductFormHooks(groupId, foodProducts, addFoodProducts, setFoodProductsData);
        ...
        return (
            <div className="flex flex-col gap-6">
                {foodProducts.map((product) => (
                    <div key={product.id} className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                        <div className="flex flex-col gap-4">
                            <div>
                                <h3 className="text-base font-medium text-gray-900 mb-1">販売品名</h3>
                                <p className="text-gray-700">{product.name}</p>
                            </div>
                            <div>
                                <h3 className="text-base font-medium text-gray-900 mb-1">酒類ですか？</h3>
                                <p className="text-gray-700">{product.isAlcohol ? 'はい' : 'いいえ'}</p>
                            </div>
                            <div>
                                <h3 className="text-base font-medium text-gray-900 mb-1">調理の有無</h3>
                                <p className="text-gray-700">{product.hasLicense ? '有り' : '無し'}</p>
                            </div>
                            <div>
                                <h3 className="text-base font-medium text-gray-900 mb-1">1日目の販売予定数</h3>
                                <p className="text-gray-700">{product.day1Quantity}</p>
                            </div>
                            <div>
                                <h3 className="text-base font-medium text-gray-900 mb-1">2日目の販売予定数</h3>
                                ...
                                <div className="flex w-full flex-col items-start justify-center gap-6">
                                    <div className="relative w-96">
                                        <TextBox
                                            label="販売品名"
                                            value={product.name || ''}
                                            onChange={(value) => setValue(`products.${index}.name`, value)}
                                            required
                                    ...
                                    <div className="relative w-96">
                                        <TextBox
                                            ...
                                            onChange={(value) => setValue(`products.${index}.day1Quantity`, value)}
                                            required
                                            note="半角数字"
                                    ...
                                    <div className="relative w-96">
                                        <TextBox
                                            ...
                                            onChange={(value) => setValue(`products.${index}.day2Quantity`, value)}
                                            required
                                            note="半角数字"

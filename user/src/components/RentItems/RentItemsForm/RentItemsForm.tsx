...
const RentItemsForm: FC = () => {
    ...
    if (isLoading) {
        return <div className="w-full text-center py-4">
            <p>データを読み込み中です...</p>
        </div>;
    }

    if (hasError) {
        return <div className="w-full bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative">
    ...
    return (
        <form className="w-full flex flex-col gap-8 text-font" onSubmit={onSubmit}>
            <div>
                <p className="mb-1 text-sm text-alert mb-[4px]">会場申請を先に申請してください。</p>
                <br/>
                <Radio
                    label="物品申請を行いますか？"
                    value={hasItems ? "1" : "0"}
                        ...
                        <FormContainer key={index}>
                            ...
                            <div>
                                <Selector
                                    ...
                                    required
                                    options={Array.from({length: 20}, (_, i) => ({id: i + 1, name: `${i + 1}`}))}
                                    error={errors?.items?.[index]?.count}
                                />
                                <p className="text-xs text-sub">※必要最低限の数だけ申請してください</p>
                            </div>
                            <div>
                                <p className="text-xs text-font">使用する個数が20個以上の場合はメールをお送りください<br/>nutfes.soumu@gmail.com
                                </p>
                            </div>

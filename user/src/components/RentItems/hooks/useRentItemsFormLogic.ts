...
export const useRentItemsFormLogic = () => {
    ...
    const [itemList, setItemList] = useState<ItemForm[]>([{ itemId: '', count: 1 }]);
    const [errors, setErrors] = useState<FormErrors | null>(null);
    const [isValid, setIsValid] = useState<boolean>(false);
    const [submitError, setSubmitError] = useState<string>('');

    // 会場タイプに基づいて適切なエンドポイントを選択
    const itemsEndpoint = locationType === '1'
    ...
    } = useApiGet<ApiResponse<RentalOrder[]>>(`${API_ENDPOINTS.RENTAL_ORDERS}?group_id=${currentGroupId}`);
        ...
        if (rentalOrders.length > 0) {
            ...
            if (itemsResponse?.data && itemsResponse.data.length > 0) {
                ...
                const isInsideOnly = rentalOrders.every(order => {
                    const item = itemsResponse.data.find(i => i.id === order.rental_item_id);
    ...
    const onSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            if (!hasItems) {
                if (rentalOrders.length > 0) {
                    // 既存のデータをすべて削除
                    const result = await deleteRentalOrders(rentalOrders.map(item => item.id));

                    if (result.success) {
                        alert('物品申請を取り消しました。');
                        mutateRentalOrders();
                    } else {
                        setSubmitError('削除中にエラーが発生しました。もう一度お試しください。');
                    }
                } else {
                    alert('物品申請がありません。');
                }
